...
"""Created on Sun Mar 26 06:17:54 MDT 2023

Synopsis:

disease_cosine_similarity.py --query patient_hpo.tsv \
                             --corpus disease_phenotype.tsv

Description:

A script to calculate the cosine similarity between a patient HPO
phenotype list and an HPO phenotype corpus where the corpus contains
gene/phenotype or disease/phenotype associations.

Args:

  --query, -q <patient_hpo.tsv>

    A text file of HPO phenotype IDs for the patient.  The file can be
    a multi-column tab-delimited file.  The first column must be HPO
    identifiers (e.g. HP:0001943) with one HPO ID on each line.
    Additional columns will be ignored.

  --corpus, -c <disease_phenotype.tsv>

    A tab-delimited text file of LABEL/phenotype associations where
    LABEL is disease, gene or any other grouping label.  Each row in
    the file has at least two tab-delimited columns (additional
    columns will be ignored).  The first column is the label and the
    second column is an HPO identifiers (e.g. HP:0001943).
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='A script to calculate the cosine similarity between a patient HPO phenotype list and an HPO phenotype corpus where the corpus contains gene/phenotype or disease/phenotype associations.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--query', '-q',
                        help='An list of HPO identifiers for the query')
    parser.add_argument('--corpus', '-c',
                        help='An TSV of [disease|gene]/phenotype associations')
    args = parser.parse_args()


    # Read LABEL/phenotype corpus
    corpus_df = pd.read_table('disease_name_hpo_list.tsv')
    #diseases = df['hpo']
